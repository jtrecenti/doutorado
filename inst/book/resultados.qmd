...
Nesta seção são revelados os resultados das simulações realizadas. Como comentado anteriormente, foram realizadas `r total_sim` simulações no total, variando no tipo de Captcha, a acurácia do modelo inicial e a quantidade de tentativas no oráculo.
...
Separando os resultados gerais por quantidade de tentativas, observa-se os ganhos relativos e absolutos nas [Figuras @fig-simulacao-geral-ntry-relativo] e [-@fig-simulacao-geral-ntry-absoluto]. Cada ponto é uma simulação e o ponto em destaque é o valor médio, acompanhado de intervalo $m \mp 2*s/\sqrt(n)$, com $m$ sendo a média, $s$ o desvio padrão e $n$ a quantidade de dados. A linha pontilhada indica se a acurácia aumentou ou diminuiu após a aplicação da técnica. 
...
A @fig-simulacao-geral-inicial-relativo mostra os ganhos relativos. É possível notar uma tendência de queda no ganho de acurácia com uso do oráculo conforme aumenta a acurácia do modelo inicial. Esse resultado é esperado, pois, como a acurácia é um número entre zero e um, um modelo que já possui alta acurácia não tem a possibilidade de aumentar tanto.
...
dados_iteracao_trf5 |> 
  ...
  ggplot2::geom_col(fill = viridis::viridis(2,.9,.3,.7,1,"A")[1]) +
  ggplot2::scale_y_continuous(labels = scales::percent) +
